...
public class ConstraintValidationTest {
    public static void test() {
        TestHelper.printSectionHeader("5. 제약 조건 검증 (Constraint Validation)");
        TestHelper.printDescription("HTML5 표준 제약 조건 검증 API:");
        TestHelper.printDescription("- checkValidity(): 유효성 검사");
        TestHelper.printDescription("- reportValidity(): 유효성 검사 + 에러 표시");
        TestHelper.printDescription("- validity: ValidityState 객체");
        ...
        // checkValidity() test
        TestHelper.addExampleCode(constraintSection,
        ...
        TestHelper.assertTrue("checkValidity: 올바른 이메일은 valid", checkValidityField.checkValidity());

        checkValidityField.value = "잘못된이메일";
        TestHelper.assertFalse("checkValidity: 잘못된 이메일은 invalid", checkValidityField.checkValidity());
        ...
        // Validation tests
        TestHelper.assertFalse("reportValidity: 빈 필수 필드는 false 반환", reportValidityField.reportValidity());

        reportValidityField.value = "값 입력";
        TestHelper.assertTrue("reportValidity: 값이 있으면 true 반환", reportValidityField.reportValidity());
        ...
        TestHelper.assertTrue("pattern + suffix: 'johndoe'는 유효한 패턴", patternValid);

        // Test 6: Pattern mismatch
        emailPattern.value = "invalid@email";
        var patternInvalid = !emailPattern.checkValidity();
        TestHelper.assertTrue("pattern + suffix: '@' 포함 시 패턴 불일치", patternInvalid);
        ...
        TestHelper.assertNotNull("ValidityState: validity 객체가 존재해야 함", stateCheck.validity);
    }
}
